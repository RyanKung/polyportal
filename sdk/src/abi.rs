...
impl AbiEncoder {
    ...
    pub fn encode_string(s: &str) -> Vec<u8> {
        ...
        let _data_length = padded_length + 64; // offset + length + data

        let mut result = Vec::new();

        // Offset to data (32 bytes)
        result.extend_from_slice(&Self::encode_uint256(offset));

        // String length (32 bytes)
        result.extend_from_slice(&Self::encode_uint256(length));
